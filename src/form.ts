...
const restore = (d: any) => d;
const remember = (d: any, k: string) => ({ k, d });

export default function useForm(...args) {
	const rememberKey = typeof args[0] === "string" ? args[0] : null;
	const data = (typeof args[0] === "string" ? args[1] : args[0]) || {};
	const restored = rememberKey ? restore(rememberKey) : null;
	let defaults = cloneDeep(data);
	let cancelToken: string|null = '';
	let recentlySuccessfulTimeoutId = 0;
	...
	watch(
		...
		(newValue) => {
			...
			if (rememberKey) {
				remember(cloneDeep(newValue.__remember()), rememberKey);
			}
		},
